...
// Defensive image generation layer.
//
// Design goals (in priority order):
//   1. Always produce SOME image if any configured model can produce one.
//   2. Send the minimum request body that's guaranteed to work.
//   3. Recover silently from parameter-shape drift (new model rejected a param).
//   4. Fall back to a known-good model when the user's preferred one breaks.
//   5. Never crash on a recoverable error — only surface auth/network failures.
...
async function attemptOnce(provider: string, apiKey: string, model: string, prompt: string, signal: AbortSignal): Promise<GeneratedImage> {
...
// errors, advance to the next model. On hard errors (auth, content policy),
// bail immediately — those won't get better with a different model.
export async function generateImageWithFallback(req: ImageGenerationRequest): Promise<GeneratedImage> {
  const apiKey = getKey(req.provider)
