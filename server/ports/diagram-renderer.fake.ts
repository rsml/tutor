 ...
 * diagram source a real renderer would reject. Every other chart succeeds
 * with a synthetic, clearly fake markup string that echoes the chart's
 * index, so ordering is easy to assert on without depending on real
 * kroki.io or Electron markup shapes.
 */
  ...
  return {
    ...
    async render(charts: string[]) {
      ...
      return charts.map((chart, i) =>
        chart.trim().length === 0 ? '' : `<svg data-fake-diagram-index="${i}"></svg>`,
      )
    },
  }
}
