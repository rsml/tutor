...
interface DeleteBookDialogProps {
  open: boolean
  payload: Extract<LibraryDialog, { kind: 'delete' }> | null
 ...
 * into the fade. */
export function DeleteBookDialog({ open, payload, dispatch, mutating, onConfirm }: DeleteBookDialogProps) {
  return (
    <Dialog open={open} onOpenChange={open => { if (!open) dispatch({ type: 'close' }) }}>
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription>
            Are you sure you want to delete &ldquo;{payload?.book.title}&rdquo;? Type <strong>delete</strong> to confirm.
          </DialogDescription>
        </DialogHeader>
        <input
          value={payload?.input ?? ''}
        ...
        <DialogFooter>
          ...
          <Button variant="destructive" onClick={onConfirm} disabled={payload?.input.toLowerCase() !== 'delete' || mutating}>OK</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
