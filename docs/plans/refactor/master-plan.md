...
|---|---|---|---|
| 0 | `phase-0-safety-net` | Characterization tests (fastify.inject), buildServer() extraction, CI, lefthook, zero lint warnings, hygiene (.DS_Store, talks/ → rsml/talks, stale plans, cruft dirs, pnpm-workspace.yaml), 6 tracking issues | ≥229 tests green twice (232 actual), CI green, hygiene greps clean |
...
Phase 0's gate originally read ≥419 tests, derived from a 384-test baseline that had been measured in a checkout holding nested full repo copies under `.worktrees/` and `.claude/worktrees/`. Vitest was collecting their test files as if they belonged to this project, and 18 of the 33 files did not. The true baseline is 194 tests in 15 files, so the target became ≥229. See `phase-0.md` for the full correction and the `vitest.config.ts` exclude that prevents a recurrence.
